...
class ProjectColumnarSuite extends WholeStageTransformerSuite {
  ...
  test("test nondeterministic function input_file_name") {
    ...
    assert(
      df.queryExecution.executedPlan
        .find(p => p.isInstanceOf[ProjectColumnarExec])
        .isEmpty)
  }
  ...
  test("udf in agg has 1 column") {
    runQueryAndCompare(
      """select sum(hash(plus_one(l_extendedprice)) *l_discount + hash(l_orderkey)
        | + hash(l_partkey) + hash(l_suppkey) + hash(l_linenumber) + hash(l_comment)
        | + hash(l_shipinstruct)) as revenue
        | from lineitem;""".stripMargin) {
      checkGlutenOperatorMatch[ProjectColumnarExec]
    }
  }

  test("udf in agg has 2 column") {
    runQueryAndCompare(
      """select max_by(hash(plus_one(l_extendedprice)) *l_discount + hash(l_orderkey)
        | + hash(l_partkey) + hash(l_suppkey) + hash(l_linenumber) + hash(l_comment)
        | + hash(l_shipinstruct), l_extendedprice) as revenue
        | from lineitem;""".stripMargin) {
      checkGlutenOperatorMatch[ProjectColumnarExec]
    }
  }
